# Инструкция по использованию библиотеки SatelliteProcessor для Google Earth Engine
...
7. [Гармонический анализ временных рядов](#гармонический-анализ-временных-рядов)
8. [Создание композитов](#создание-композитов)
9. [Типичные сценарии использования](#типичные-сценарии-использования)
10. [Советы по оптимизации](#советы-по-оптимизации)
...
4. **timeSeries.harmonicRegression()** - гармонический анализ сезонности и тренда по индексу
...
#### timeSeries.harmonicRegression(collection, options)
Аппроксимирует временной ряд индекса линейным трендом и годовой гармоникой. Это удобно для поиска устойчивых сезонных паттернов, трендов деградации/восстановления и оценки ожидаемой даты пика вегетации.

**Основные выходные бэнды:**
- `NDVI_intercept` — базовый уровень индекса в начале ряда
- `NDVI_trend` — линейный тренд за год
- `NDVI_amplitude` — амплитуда сезонных колебаний
- `NDVI_phase` — фаза сезонной волны
- `NDVI_peakDOY` — ожидаемый день года сезонного пика
- `NDVI_count` — число наблюдений, использованных в модели

**Пример:**
```javascript
var harmonic = SatelliteProcessor.timeSeries.harmonicRegression(
...
Map.addLayer(
  harmonic.select('NDVI_trend'),
  {min: -0.05, max: 0.05, palette: ['red', 'white', 'green']},
  'NDVI Trend'
);
Map.addLayer(
  harmonic.select('NDVI_peakDOY'),
  {min: 90, max: 280, palette: ['purple', 'cyan', 'yellow']},
  'NDVI Peak DOY'
);
```
